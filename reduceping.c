...
int is_reduceping_enabled() {
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, ROOT_KEY, 0, KEY_READ, &intkey) != ERROR_SUCCESS) {
    ...
    DWORD numsubkeys;
    if (RegQueryInfoKey(intkey, NULL, NULL, NULL, &numsubkeys, NULL, NULL, NULL, NULL, NULL, NULL, NULL) != ERROR_SUCCESS) {
    ...
    for (i = 0; i < numsubkeys; i++) {
        ...
        if (RegEnumKey(intkey, i, subkey + strlen(ROOT_KEY), sizeof(subkey)) != ERROR_SUCCESS) {
...
void enable_reduceping() {
    ...
    for (i = 0; i < numsubkeys; i++) {
        ...
        if (RegSetValueEx(tcpkey, "TcpAckFrequency", 0, REG_DWORD, (const BYTE *) &value, sizeof(value)) != ERROR_SUCCESS) {
            MessageBox(NULL, "ReducePing encountered an error while setting the TcpAckFrequency registry value, attempting to continue.", "ReducePing", MB_ICONEXCLAMATION | MB_OK);
            RegCloseKey(tcpkey);
            continue;
        }
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    /* show window and start update loop */
